...
const {
  ...
  + 'FN:@𝗝𝗼𝘁𝗮𝗥𝗼𝘆𝗮𝗹\n' // Nama
  + 'ORG:𝗝𝗼𝘁𝗮𝗥𝗼𝘆𝗮𝗹-Bot;\n' // Nama bot
  + 'TEL;type=CELL;type=VOICE;waid=34644363340:+34 644364340n' // Nomor bot
  + 'END:VCARD' 
  ...
  *〘 TEXTO2 〙*
  ...
  ┃ _Ex : ${prefix}Ephoto 25 𝗝𝗼𝘁𝗮𝗥𝗼𝘆𝗮𝗹_
  ┠⊷️ *${prefix}Textpro list*
  ┃ _Lista de textpro
  ┠⊷️ *${prefix}Textpro (code + Texto)*
  ┃ _Ex : ${prefix}Textpro 55 𝗝𝗼𝘁𝗮𝗥𝗼𝘆𝗮𝗹_
  ┗━━━━━━━━━━━━━━━━━━━━
  *〘 IMAGENES 〙*
  ┏━━━━━━━━━━━━━━━━━━━━
  ┠⊷️ *${prefix}Cogan*
  ...
  *OVO :* _${Ovo}_`,
  ...
  pok = await getBuffer(nimek)
  Lxa.sendMessage(from, pok, image, {
	quoted: mek, caption: `𝗝𝗼𝘁𝗮𝗥𝗼𝘆𝗮𝗹-Bot`
  })
					...
					case 's':
					  if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						  ...
						  await ffmpeg(`./${media}`)
							  ...
							  .on('end', function () {
								  console.log('Finish')
								  exec(`webpmux -set exif ${addMetadata('*@JotaRoyal*', 'STIC.KERS')} ${ran} -o ${ran}`, async (error) => {
						  ...
						  await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							  ...
							  exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								  fs.unlinkSync(ranp)
								  if (err) return reply(mess.error.stick)
								  exec(`webpmux -set exif ${addMetadata('*@JotaRoyal*', 'STIC.KERS')} ${ranw} -o ${ranw}`, async (error) => {
									  if (error) return reply(mess.error.stick)
  ...
  //--Hilih maker
				  ...
				  if (!isRegister) return reply(mess.only.daftarB)
					  ...
					  exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						  fs.unlinkSync(ranp)
						  if (err) return reply(mess.error.stick)
						  exec(`webpmux -set exif ${addMetadata('*@JotaRoyal*', 'STIC.KERS')} ${rano} -o ${rano}`, async (error) => {
							  if (error) return reply(mess.error.stick)
  ...
  //--Convert stiker to image
				  case 'toimg':
					  ...
					  exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						  ...
						  buffer = fs.readFileSync(ran)
						  Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: '𝗝𝗼𝘁𝗮𝗥𝗼𝘆𝗮𝗹-Bot'})
						  fs.unlinkSync(ran)
					  })
					  break
